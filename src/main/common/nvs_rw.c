...
esp_err_t nvs_read_buffer(const char* name, uint8_t** buffer, size_t * buffer_size) {
    ...
    if (err != ESP_OK) {
    	LOGE(LOG_NWS_RW, "Cant open namespace %s for paramter %s: %d", STORAGE_NAMESPACE, name, err);
    	return err;
    }
    ...
    *buffer_size = 0;
    ...
    if (err != ESP_OK || *buffer_size == 0) {
    	LOGE(LOG_NWS_RW, "Cant read blob size for blob %s from NVS: %d", name, err);
    ...
    *buffer = (uint8_t *) malloc(*buffer_size);
    if (*buffer == NULL) {
    	LOGE(LOG_NWS_RW, "Cant allocate %d bytes. OOM.", *buffer_size);
    ...
    if (err != ESP_OK) {
    	LOGE(LOG_NWS_RW, "Cant read blob %s from NVS: %d", name, err);
...
esp_err_t nvs_write_buffer(const char* name, const uint8_t* buffer, size_t buffer_size) {
    ...
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
    	LOGE(LOG_NWS_RW, "Cant cleanup memory for %s: %d", name, err);
    ...
    if (err) {
    	LOGE(LOG_NWS_RW, "Cant commit cleanup changes for %s: %d", name, err);
    ...
    if (err != ESP_OK) {
    	LOGE(LOG_NWS_RW, "Cant set blob for %s [%d bytes]: %d", name, buffer_size, err);
    ...
    err = nvs_commit(handle);
    if (err) {
    	LOGE(LOG_NWS_RW, "Cant commit setblob changes for %s [%d bytes]: %d", name, buffer_size, err);
