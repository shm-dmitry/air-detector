...
bool i2c_init_driver(i2c_port_t port, int gpio_sda, int gpio_scl){
    ...
    if (port == I2C_NUM_0) {
    	...
    	if (mutex_0 == NULL) {
    	    ESP_LOGE(LOG_I2C, "Cant init semaphore#0");
    		return false;
    	}
    } else {
    	...
    	if (mutex_1 == NULL) {
    	    ESP_LOGE(LOG_I2C, "Cant init semaphore#1");
    		return false;
    	}
    }

    ESP_LOGI(LOG_I2C, "I2C port %d with pins sda %d / scl %d initialized", port, gpio_sda, gpio_scl);
...
void i2c_register_port(i2c_port_t port, int gpio_sda, int gpio_scl) {
	switch (port) {
		...
		case I2C_NUM_1:
			...
			break;
		default:
			ESP_LOGE(LOG_I2C, "Unknown I2C port requested: %d", port);
			break;
	}
}
...
i2c_handler_t * i2c_get_handlers(i2c_port_t port){
	switch (port) {
		case I2C_NUM_0:
			if (!port_0_registered) {
				ESP_LOGE(LOG_I2C, "I2C port %d not initialized yet", port);
				return NULL;
			}
			break;
		case I2C_NUM_1:
...
esp_err_t i2c_read(i2c_port_t port, SemaphoreHandle_t mutex, uint8_t addr_id, uint8_t* buffer, uint8_t buffer_size) {
#if I2C_DEBUG_OUTPUT
	ESP_LOGI(LOG_I2C, "i2c_read send request to port %d / addr %02x", port, addr_id);
#endif
    ...
    if (ret) {
    	ESP_LOGE(LOG_I2C, "i2c_read Cant write address %d to port %d : %d", addr_id, port, ret);
    ...
    if (buffer_size > 1) {
		...
		if (ret) {
			ESP_LOGE(LOG_I2C, "i2c_read Cant read data from port %d addr %02x : %d", port, addr_id, ret);
			i2c_cmd_link_delete(cmd);
			return ret;
		}
    }
	...
	if (xSemaphoreTake(mutex, I2C_MUTEX_AWAIT) != pdTRUE) {
		ESP_LOGE(LOG_I2C, "i2c_read port %d take mutex timeout", port);
    ...
    if (ret) {
    	ESP_LOGE(LOG_I2C, "i2c_read exec command faileod to port %d addr %02x : %d", port, addr_id, ret);
    }
...
#if I2C_DEBUG_OUTPUT
    if (ret == ESP_OK) {
    	ESP_LOGI(LOG_I2C, "i2c_read received buffer to port %d / addr %02x", port, addr_id);
...
esp_err_t i2c_write(i2c_port_t port, SemaphoreHandle_t mutex, uint8_t addr_id, const uint8_t* buffer, uint8_t buffer_size) {
#if I2C_DEBUG_OUTPUT
	ESP_LOGI(LOG_I2C, "i2c_write sending buffer to port %d / addr %02x", port, addr_id);
#endif
    ...
    if (ret) {
    	ESP_LOGE(LOG_I2C, "Cant write address %d to port %d : %d", addr_id, port, ret);
    	i2c_cmd_link_delete(cmd);
    	return ret;
    }
    ...
    if (buffer && buffer_size) {
        ...
        if (ret) {
        	ESP_LOGE(LOG_I2C, "Cant send buffer port %d : %d", port, ret);
        	i2c_cmd_link_delete(cmd);
        	return ret;
        }
    }
	...
	if (xSemaphoreTake(mutex, I2C_MUTEX_AWAIT) != pdTRUE) {
		ESP_LOGE(LOG_I2C, "i2c_write port %d addr_id %d take mutex timeout", port, addr_id);
    	i2c_cmd_link_delete(cmd);
		return ESP_ERR_TIMEOUT;
	}
    ...
    ret = i2c_master_cmd_begin(port, cmd, I2C_COMMAND_AWAIT);
    if (ret) {
    	ESP_LOGE(LOG_I2C, "i2c_write exec command failed to port %d addr %02x : %d", port, addr_id, ret);
    }
...
esp_err_t i2c_write_read(i2c_port_t port, SemaphoreHandle_t mutex, uint8_t addr_id, const uint8_t* write_buffer, uint8_t write_buffer_size, uint8_t* read_buffer, uint8_t read_buffer_size) {
#if I2C_DEBUG_OUTPUT
	ESP_LOGI(LOG_I2C, "i2c_write_read send request to port %d / addr %02x", port, addr_id);
#endif
	...
	esp_err_t ret = i2c_master_write_byte(cmd, (addr_id << 1) | I2C_MASTER_WRITE, true);
	if (ret) {
		ESP_LOGE(LOG_I2C, "i2c_write_read Cant write address %d to port %d : %d", addr_id, port, ret);
		i2c_cmd_link_delete(cmd);
		return ret;
	}
	...
	if (write_buffer && write_buffer_size) {
		...
		if (ret) {
			ESP_LOGE(LOG_I2C, "i2c_write_read Cant write data to port %d addr %02x : %d", port, addr_id, ret);
			i2c_cmd_link_delete(cmd);
			return ret;
		}
	}
	...
	if (ret) {
		ESP_LOGE(LOG_I2C, "i2c_write_read Cant read address %d from port %d : %d", addr_id, port, ret);
		i2c_cmd_link_delete(cmd);
		return ret;
	}
	...
	if (read_buffer_size > 1) {
		...
		if (ret) {
			ESP_LOGE(LOG_I2C, "i2c_write_read Cant read data from port %d addr %02x : %d", port, addr_id, ret);
			i2c_cmd_link_delete(cmd);
			return ret;
		}
	}
	...
	if (xSemaphoreTake(mutex, I2C_MUTEX_AWAIT) != pdTRUE) {
		ESP_LOGE(LOG_I2C, "i2c_write_read port %d take mutex timeout", port);
		return ESP_ERR_TIMEOUT;
	}
	...
	ret = i2c_master_cmd_begin(port, cmd, I2C_COMMAND_AWAIT);
	if (ret) {
		ESP_LOGE(LOG_I2C, "i2c_write_read exec command faileod to port %d addr %02x : %d", port, addr_id, ret);
	}
...
#if I2C_DEBUG_OUTPUT
	if (ret == ESP_OK) {
		ESP_LOGI(LOG_I2C, "i2c_write_read received buffer to port %d / addr %02x", port, addr_id);
...
esp_err_t i2c_general_reset(i2c_handler_t * i2c) {
	...
	if (res) {
		ESP_LOGE(LOG_I2C, "i2c_general_reset error %d", res);
	} else {
		ESP_LOGI(LOG_I2C, "i2c_general_reset done.");
	}
	return res;
}
